...
public protocol ComponentWithDelegate: Component {
     ...
     * - a good idea is to create an `enum`, cases without value should be used with method `ObservableConvertibleType.rewrite` and cases with should be used with method `ObservableConvertibleType.map`
     * ## Example
     ...
     * - WARNING: When listening to Component's actions, subscribe to `Component.action` instead of this variable. `Component.action` includes `Component.perform(action:)` calls as well as `Observable`s defined in `actions`.
     */
     ...
     * ## Example
     * You decide to change the amount of active buttons based on device orientation. `actions` don't automatically change based on the conditions that are in the computed variable, because this would be ineffective. This is where you call `resetActions()` when orientation changes causing `actions` to update and the amount of active buttons is changed as well.
     */
    func resetActions()
}
